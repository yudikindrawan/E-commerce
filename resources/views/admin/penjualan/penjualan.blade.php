    ...
    <section class="sidebar">
      ...
      <ul class="sidebar-menu" data-widget="tree">
        ...
        <li class="treeview">
          ...
          <ul class="treeview-menu">
            <li class="active"><a href="{{route('penjualan')}}"><i class="fa fa-circle-o"></i>Data Penjualan</a></li>
                ...
                <div class="col-xs-12">
                  <div class="box" style="box-shadow: 0 0 5px rgba(0,0,0,0.1); border-top:0px;">
                    <div class="box-header">
                      <div class="row">
                        <div class="col-xs-7">
                          <a href="#" class="modalCreateTrigger" data-toggle="modal" data-target="#modalCreateTrigger"><button class="btn btn-success">Tambah Penjualan</button></a><input type="hidden" name="_token" value="{{ csrf_token() }}">
                        </div>
                      </div>
                    </div>
                    <!-- /.box-header -->
                        ...
                        <table id="example" class="table table-striped">
                          ...
                          <tr>
                            <td>{{$penjualan->produk->nama_produk}}</td>
                              ...
                              <td> <a onClick="modalEditTriger({{$penjualan->id}})" class="btn btn-primary" data-toggle="tooltip" title="Ubah"><i class="fa fa-pencil"></i></a>
                                   <a onClick="modalReturTrigger({{$penjualan->id}})" class="btn btn-info" data-toggle="tooltip" title="Retur"><i class="fa fa-file-text">
                              </i></a> 
...
<script type="text/javascript">
 ...
 function modalinvoiceTrigger(id){
      $.ajax({
        url     : "{{route('invoice_penjualan')}}",
        method  : 'get',
        data    : {
          'id' : id
        },
 ...
 function modalReturTrigger(id){
      $.ajax({
        url     : "{{route('modal_retur_penjualan')}}",
        method  : 'get',
        data    : {
          'id' : id
        },
...
// modal create
    ...
    event.preventDefault();
        $.ajax({
            url     : "{{route('modal_create_penjualan')}}",
            method  : 'get',
            success : function(response){
                $('.modalKu').html(response);
                $('#myModal').modal({ backdrop: 'static', keyboard: false });
...
// MODAL EDIT
    function modalEditTriger(id){
      $.ajax({
        url     : "{{route('modal_edit_penjualan')}}",
        method  : 'get',
        data    : {
          'id' : id
        },
